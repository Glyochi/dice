...
ITERATIONS=100000
PROGRESS_ANNOUNCEMENT_PER_ITERATIONS=100000
# Make sure there are at least 10 announcements
if [[ $((ITERATIONS / PROGRESS_ANNOUNCEMENT_PER_ITERATIONS)) -lt 10 ]]; then
  PROGRESS_ANNOUNCEMENT_PER_ITERATIONS=$((ITERATIONS / 10))
fi
MAX_PROB_LENGTH=80
...
while [[ $iter -lt $ITERATIONS ]]; do
  ...
  # Report progress
  if [[ $((iter % PROGRESS_ANNOUNCEMENT_PER_ITERATIONS)) -eq 0 ]]; then
